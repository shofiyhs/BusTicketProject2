...
<body>
    ...
    // var_export($_SESSION);
    // echo "</pre>";
    $sqlRoutes = "SELECT `route_cities`,COUNT(*) AS route_count FROM `routes` GROUP BY `route_dep_date`";

$resultRoutes = mysqli_query($conn, $sqlRoutes);

$chartDataRoutes = [];
while ($rowRoutes = mysqli_fetch_assoc($resultRoutes)) {
    $chartDataRoutes['labels'][] = $rowRoutes['route_cities'];
    $chartDataRoutes['data'][] = $rowRoutes['route_count'];
}

$chartDataRoutesJson = json_encode($chartDataRoutes);
echo $chartDataRoutesJson
    ?>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        const chartData = <?php echo $chartDataRoutesJson; ?>;
